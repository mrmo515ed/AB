...
const target = '      }, (e)=>console.warn("Chat sync warning:", e));    } catch(err){}  }}window.ensureChatListener = ensureChatListener;function openChat(id){';
if (code.includes(target)) {
  code = code.replace(target, `      }, (e)=>console.warn("Chat sync warning:", e));    } catch(err){}  }}
      ...
      window.getDocs(qMsgs).then(snap => {
        if(snap && !snap.empty) {
          ...
          const olderMsgs = snap.docs.map(d => {
            const m = d.data();
            const isMine = (m.senderId === myUid || m.fromId === myUid || (m.senderId && (m.senderId === S.me.id || m.senderId === S.me.uid)) || (!m.senderId && m.from === "me") || (typeof isMe === "function" && isMe(m.senderId)));
