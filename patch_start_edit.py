...
m = re.search(r'function startEditMsg\(mid\)\{const m=msgById\(mid\);if\(!m\)return;S\.editMid=mid;S\.selMode=false;S\.selIds=\[\];render\(\);setTimeout\(\(\)=>\{const i=\$\(\'#minp\'\);if\(i\)\{i\.value=m\.text\|\|"";i\.focus\(\)\}\},50\)\}', text)
if m:
    rep = '''function startEditMsg(mid){
  const m=msgById(mid);if(!m)return;
  S.editMid=mid;S.selMode=false;S.selIds=[];
